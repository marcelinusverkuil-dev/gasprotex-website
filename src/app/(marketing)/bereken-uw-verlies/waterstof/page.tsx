...
const WATERSTOF_TYPES = [
  { id: 'grijs', label: 'Grijs', prijs: 0.27, omschrijving: 'Geproduceerd uit aardgas zonder CO₂-afvang' },
  { id: 'blauw', label: 'Blauw', prijs: 0.41, omschrijving: 'Aardgas met CO₂-afvang en -opslag (CCS)' },
  { id: 'groen', label: 'Groen', prijs: 0.99, omschrijving: 'Elektrolyse op basis van hernieuwbare energie' },
]

export default function WaterstofCalculatorPage() {
  const [type, setType] = useState('grijs')
  ...
  const huidigType = WATERSTOF_TYPES.find(t => t.id === type)!
  ...
  const handleTypeChange = (id: string) => {
    setType(id)
    const t = WATERSTOF_TYPES.find(t => t.id === id)!
    ...
    <>
      <section style={{ background: '#ffffff', paddingTop: '100px', paddingBottom: '40px' }}>
        <div className="max-w-7xl mx-auto" style={{ paddingLeft: '80px', paddingRight: '80px' }}>
          <Link href="/bereken-uw-verlies" className="inline-flex items-center gap-2 text-[#7AADCC] hover:text-[#C2DCE8] mb-6 transition-colors" style={{ fontSize: 13 }}>
            <svg width="14" height="10" viewBox="0 0 14 10" fill="none" style={{ transform: 'rotate(180deg)' }}>
              <path d="M8.5 1L13 5L8.5 9M1 5H13" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            Alle gassoorten
          </Link>
          <p className="text-[#7AADCC] mb-3" style={{ fontSize: 13, letterSpacing: '1.5px', textTransform: 'uppercase' }}>Waterstof</p>
          <h1 className="font-bold text-[#0F2D4B]" style={{ fontSize: 'clamp(32px, 4vw, 52px)', lineHeight: 1.2, letterSpacing: '-0.02em' }}>
            Lekkostencalculator waterstof
          </h1>
          <p className="text-[#4A6880] mt-4" style={{ fontSize: 17, lineHeight: 1.7, maxWidth: 620 }}>
            Bereken wat waterstoflekkages u jaarlijks kosten — voor grijs, blauw én groen waterstof.
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            <div className="rounded-2xl" style={{ background: 'linear-gradient(to bottom right, #0F2D4B, #1E5A8A)', padding: '48px' }}>
              ...
              <div className="flex flex-col gap-6">
                ...
                <div>
                  <label className="block text-[#C2DCE8] mb-3" style={{ fontSize: 14 }}>Type waterstof</label>
                  <div className="flex gap-3">
                    {WATERSTOF_TYPES.map(t => (
                      <button key={t.id} onClick={() => handleTypeChange(t.id)} className="flex-1 rounded-lg text-left transition-all"
                        style={{ background: type === t.id ? 'rgba(217,119,55,0.25)' : 'rgba(255,255,255,0.06)', border: type === t.id ? '1px solid #D97737' : '1px solid rgba(255,255,255,0.12)', padding: '12px 14px' }}>
                ...
                <div>
                  <label className="block text-[#C2DCE8] mb-2" style={{ fontSize: 14 }}>
                    Gasprijs (€/m³) <span className="text-[#7AADCC] ml-2" style={{ fontSize: 12 }}>aanpasbaar</span>
                  </label>
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="rounded-2xl" style={{ background: 'linear-gradient(to bottom right, #0F2D4B, #1E5A8A)', padding: '28px 32px' }}>
                ...
                <p className="text-[#7AADCC] mt-1" style={{ fontSize: 13 }}>m³ waterstof per jaar</p>
              </div>
